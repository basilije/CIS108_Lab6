...
int main()
{
	...
	string temp_string;  // Initializing a string variable

	new_string.clear();  // Clear the new string

	for (unsigned int i = size(original_string); i > 0; i--)	// Loop backward through the static string
	{
		temp_string = original_string[i-1];  // taking element from the original string
		new_string.push_back(temp_string);  // add that element to the end of a vector new_string
	}
	...
	for (itr = new_string.begin(); itr < new_string.end(); itr++)  // iterate a vector
		cout << *itr;  // print iterated vector element
